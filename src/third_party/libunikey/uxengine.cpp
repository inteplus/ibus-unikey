...
typedef int (UxEngine::* UxKeyProc)(UkKeyEvent & ev);

UxKeyProc UxKeyProcList[vneCount] = {
    &UxEngine::processRoof,    //vneRoofAll
    &UxEngine::processRoof,    //vneRoof_a
    &UxEngine::processRoof,    //vneRoof_e
    &UxEngine::processRoof,    //vneRoof_o
    &UxEngine::processHook,    //vneHookAll
    &UxEngine::processHook,    //vneHook_uo
    &UxEngine::processHook,    //vneHook_u
    &UxEngine::processHook,    //vneHook_o
    &UxEngine::processHook,    //vneBowl
    &UxEngine::processDd,      //vneDd
    &UxEngine::processTone,    //vneTone0
    &UxEngine::processTone,    //vneTone1
    &UxEngine::processTone,    //vneTone2
    &UxEngine::processTone,    //vneTone3
    &UxEngine::processTone,    //vneTone4
    &UxEngine::processTone,    //vneTone5
...
bool UxEngine::m_classInit = false;

//------------------------------------------------------------------
int UxEngine::processRoof(UkKeyEvent & ev)
{
...
//------------------------------------------------------------------
int UxEngine::processHookWithUO(UkKeyEvent & ev)
{
    VowelSeq vs, newVs;
    int i, vStart, vEnd;
    int curTonePos, newTonePos, tone;
...
//------------------------------------------------------------------
int UxEngine::processHook(UkKeyEvent & ev)
{
    if (!m_pCtrl->vietKey || m_current < 0 || m_buffer[m_current].vOffset < 0)
        return processAppend(ev);
...
//----------------------------------------------------------
int UxEngine::getTonePosition(VowelSeq vs, bool terminated)
{
...
//----------------------------------------------------------
int UxEngine::processTone(UkKeyEvent & ev)
{
...
//----------------------------------------------------------
int UxEngine::processDd(UkKeyEvent & ev)
{
...
//----------------------------------------------------------
int UxEngine::processMapChar(UkKeyEvent & ev)
{
    int capsLockOn = 0;
    int shiftPressed = 0;
    if (m_keyCheckFunc)
...
//----------------------------------------------------------
int UxEngine::processTelexW(UkKeyEvent & ev)
{
    if (!m_pCtrl->vietKey)
        return processAppend(ev);
...
//----------------------------------------------------------
int UxEngine::checkEscapeVIQR(UkKeyEvent & ev)
{
    if (m_current < 0)
        return 0;
    WordInfo & entry = m_buffer[m_current];
...
//----------------------------------------------------------
int UxEngine::processAppend(UkKeyEvent & ev)
{
    int ret = 0;
    switch (ev.chType) {
    case ukcReset:
...
//----------------------------------------------------------
int UxEngine::appendVowel(UkKeyEvent & ev)
{
...
//----------------------------------------------------------
int UxEngine::appendConsonnant(UkKeyEvent & ev)
{
...
//----------------------------------------------------------
int UxEngine::processEscChar(UkKeyEvent & ev)
{
    if (m_pCtrl->vietKey && 
...
//----------------------------------------------------------
void UxEngine::pass(int keyCode)
{
    UkKeyEvent ev;
...
//---------------------------------------------
int UxEngine::processNoSpellCheck(UkKeyEvent & ev)
{
    WordInfo & entry = m_buffer[m_current];
...
//----------------------------------------------------------
int UxEngine::process(unsigned int keyCode, int & backs, unsigned char *outBuf, int & outSize, UkOutputType & outType)
{
    UkKeyEvent ev;
    prepareBuffer();
    m_backs = 0;
    ...
    int ret;
    if (!m_toEscape) {
        ret = (this->*UxKeyProcList[ev.evType])(ev);
    }
    else {
        m_toEscape = false;
...
//----------------------------------------------------------
int UxEngine::writeOutput(unsigned char *outBuf, int & outSize)
{
    StdVnChar stdChar;
    int i, bytesWritten;
    int ret = 1;
...
//---------------------------------------------
int UxEngine::getSeqSteps(int first, int last)
{
    StdVnChar stdChar;
...
//---------------------------------------------
void UxEngine::markChange(int pos)
{
    if (pos < m_changePos) {
...
//----------------------------------------------------------------
void UxEngine::synchKeyStrokeBuffer()
{
...
//---------------------------------------------
int UxEngine::processBackspace(int & backs, unsigned char *outBuf, int & outSize, UkOutputType & outType)
{
...
//----------------------------------------------------
int UxEngine::macroMatch(UkKeyEvent & ev)
{
    int capsLockOn = 0;
    int shiftPressed = 0;
    if (m_keyCheckFunc)
...
//----------------------------------------------------
int UxEngine::restoreKeyStrokes(int & backs, unsigned char *outBuf, int & outSize, UkOutputType & outType)
{
    outType = UkKeyOutput;
...
//--------------------------------------------------
int UxEngine::processWordEnd(UkKeyEvent & ev)
{
    if (m_pCtrl->options.macroEnabled && macroMatch(ev))
        return 1;
...
//---------------------------------------------------------------------------
bool UxEngine::lastWordHasVnMark()
{
